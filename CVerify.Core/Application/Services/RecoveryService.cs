...
public class RecoveryService : IRecoveryService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (hasRecentClaim)
        {
            ...
            {
                _rateLimitPolicyService.LogBypass("Organization recovery cooldown", "SubmitClaimAsync", org.TaxCode);
            }
            else
            {
        ...
        {
            _logger.LogError(ex, "Transaction failed during recovery submission. Executing compensating transaction to clean up uploaded R2 documents. ClaimId={ClaimId}", claimId);
    ...
    public async Task<VerifyOtpResponse> VerifyRecoveryOtpAsync(VerifyOtpRequest request, string taxCode, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Recovery OTP verified successfully. ChallengeId={ChallengeId}, VerifiedEmail={VerifiedEmail}", request.ChallengeId, otpResult.Email);
