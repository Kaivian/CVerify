    ...
    <div className="flex flex-col gap-4 w-full text-left">
      ...
      {links.length > 0 ? (
        <div className="flex flex-col gap-2.5">
          {links.map((link) => {
            return (
              <div
                key={link.id}
                className="flex items-center justify-between gap-3 px-3.5 py-2.5 rounded-xl border border-separator/85 bg-field-background select-none hover:border-border transition-all group"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-8 h-8 rounded-lg bg-surface-secondary text-muted flex items-center justify-center border border-separator/50 group-hover:text-foreground group-hover:bg-surface-tertiary transition-colors shrink-0 overflow-hidden p-1.5">
                    <DynamicBrandIcon url={link.url} className="size-4 shrink-0" />
                  </div>
                  <Typography
                    type="body-xs"
                    className="font-medium text-foreground truncate font-sans text-xs"
                  >
                    {link.url}
                  </Typography>
                </div>
                <Button
                  ...
                  className="size-8 rounded-lg opacity-70 hover:opacity-100 text-danger hover:bg-danger/10 hover:border-danger/20 transition-all border border-transparent"
                >
                  <Trash2 size={14} />
                </Button>
              </div>
        ...
        <div className="flex flex-col items-center justify-center py-6 px-4 rounded-xl border border-dashed border-separator/80 bg-surface-secondary/40 select-none text-center">
          <Globe className="text-muted/65 size-5 mb-2" />
          <Typography type="body-xs" className="text-muted text-[11px] font-semibold font-outfit uppercase tracking-wider">
            No links added yet
          </Typography>
          <Typography type="body-xs" className="text-muted text-[10px] max-w-xs mt-1">
      ...
      <div className="flex flex-col gap-1.5 mt-1">
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <Input
              ...
              className="w-full px-3.5 py-2 rounded-xl border border-field-border focus:border-focus bg-field text-foreground text-xs font-semibold focus:outline-hidden cursor-pointer hover:border-border transition-all select-none focus-visible:ring-2 focus-visible:ring-focus"
            />
          </div>
          <Button
            variant="solid"
            onClick={handleAddLink}
            className="rounded-xl font-bold shrink-0 select-none hover:opacity-90 active:scale-[0.98] transition-all flex items-center justify-center gap-1.5 h-10 px-4"
          >
            <Plus size={14} />
            <span>Add</span>
          </Button>
        ...
        {error && (
          <div className="flex items-center gap-1.5 text-danger pl-1 animate-fade-in">
            <AlertCircle size={12} className="shrink-0" />
            <Typography type="body-xs" className="text-[10px] font-semibold">
              {error}
            </Typography>
          </div>
        )}
