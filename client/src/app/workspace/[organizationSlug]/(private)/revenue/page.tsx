...
import { Building2, HandCoins, AlertTriangle, ArrowLeft } from "lucide-react";
...
export default function WorkspaceRevenuePage() {
    ...
    return (
      <div className="max-w-xl mx-auto py-20 font-outfit text-foreground">
        <Card className="p-8 border border-border bg-surface text-center">
          ...
          <Typography type="body-xs" className="text-muted leading-relaxed mb-6">
            You do not have permission to access the revenue details. Financial stats and invoice data are restricted to Workspace Owners and Representatives.
          </Typography>
          <div className="flex gap-4 justify-center">
            <Button
              onClick={() => router.push(`/workspace/${organizationSlug}/dashboard`)}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-surface border border-border text-foreground select-none">
        <div className="space-y-1">
          ...
          <Typography type="body-xs" className="text-muted font-light mt-0.5">
            Workspace context: <span className="font-mono text-accent">@{organizationSlug}</span> • My Role: <span className="font-semibold text-foreground">{workspaceDetails.userRole}</span>
          </Typography>
        </div>
      ...
      <Card className="p-6 md:p-8 bg-surface border border-border rounded-2xl">
        <div className="flex items-center gap-3 mb-4 select-none">
          ...
          <div>
            ...
            <Typography type="body-xs" className="text-muted">
              Analyze partner revenue commission rates, credit payout accounts, and invoicing logs.
            </Typography>
          </div>
        </div>
        ...
        <div className="mt-8 border border-dashed border-border/80 rounded-2xl p-12 text-center select-none">
          ...
          <Typography type="body-xs" className="text-muted max-w-md mx-auto mb-6">
            The Revenue ledger and payout accounts integration is currently in sandbox testing mode. Transaction listings will populate in the next feature rollout.
          </Typography>
          <Button
            onClick={() => router.push(`/workspace/${organizationSlug}/dashboard`)}
            className="px-4 py-2 bg-foreground text-background font-bold rounded-xl text-xs cursor-pointer border-none"
